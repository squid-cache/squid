...
ClientHttpRequest::handleAdaptedHeader(Http::Message *msg)
{
    ...
    if (HttpRequest *new_req = dynamic_cast<HttpRequest*>(msg)) {
        ...
        if (request->method == Http::METHOD_CONNECT) {
            // Squid has not established a tunnel. We forward the response
            // generated by the adaptation service to the HTTP client, but the
            // service becomes responsible for the client's reaction to this
            // response. A typical use case is a service that responds with a
            // "block page" implemented as an HTTP error response.
            //
            // Since we are not going to tunnel, remove clientProcessRequest()'s
            // ban on reading post-CONNECT HTTP requests.
            if (const auto c = getConn())
