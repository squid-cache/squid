...
nfmark_t
Ip::Qos::getNfmarkFromConnection(const Comm::ConnectionPointer &conn, enum Ip::Qos::Direction direction)
{
    nfmark_t mark = 0;
    ...
    if (struct nf_conntrack *ct = nfct_new()) {
         ...
         * port numbers.
	 * For connection from client we need swap remote with local, because
	 * src = client address(remote), dst = our address(local)
         */
        auto dst = conn->remote;
        auto src = conn->local;
	if (direction == Ip::Qos::FromClient) {
            dst = conn->local;
	    src = conn->remote;
	}
                           ...
                           void *connmark)
{
    ...
    *mark = nfct_get_attr_u32(ct, ATTR_MARK);
    debugs(17, 3, "QOS: Retrieved connmark value: " << *mark);
