...
FwdState::pinnedCanRetry() const
{
    ...
    // usePinned() and usually calls connectStart(). The latter requires nil
    ...
    // XXX: FwdState never re-pins a connection. In other words, if FwdState
    // fails while using a pinned connection, we never pin a new connection to
    // the same HttpRequest::clientConnectionManager job. 2013 commit 693cb033
    // removed most re-pinning code. 2019 commit 3dde9e52 probably assumed that
    // "FTP proxying and connection-based HTTP authentication" cases mentioned
    // below might re-pin, so it added this method to avoid breaking those
    // alleged cases, but that assumption was probably wrong then and/or is
    // wrong now.
