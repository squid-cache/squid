...
aclDomainCompare(T const &a, T const &b)
{
    ...
    if (ret != 0) {
        ...
        if (ret == 0) {
            ...
            debugs(28, DBG_IMPORTANT, "WARNING: You should remove '" << (d3big?d3:d4) << "' from the ACL");
        ...
        if (strcmp(d1,d2)==0) {
            ...
            debugs(28, 2, "WARNING: You should remove one '" << d2 << "' from the ACL");
            return ret;
        }
        ...
        debugs(28, DBG_CRITICAL, "ERROR: You need to remove '" << (d1big?d1:d2) << "' from the ACL");
        self_destruct();
    }
