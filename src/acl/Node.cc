...
/// Node makers indexed by ACL type name
typedef std::map<TypeName, Maker, TypeNameCmp> Makers;

/// registered Node Makers
static Makers &
TheMakers()
{
...
/// creates an Node object of the named (and already registered) ACL child type
static
Node *
Make(TypeName typeName)
{
    ...
    Node *result = (pos->second)(pos->first);
...
{
    fatal ("unusable Node::new");
    return (void *)1;
}

void
Acl::Node::operator delete(void *)
{
    fatal ("unusable Node::delete");
}
...
{
    *name = 0;
    debugs(28, 3, "constructed, this=" << (void*)this);
}
...
Acl::Node::ParseAclLine(ConfigParser &parser, Node ** head)
{
    ...
    if ((t = ConfigParser::NextToken()) == nullptr) {
        debugs(28, DBG_CRITICAL, "FATAL: missing ACL name.");
        parser.destruct();
        return;
    }

    if (strlen(t) >= ACL_NAME_SZ) {
        debugs(28, DBG_CRITICAL, "FATAL: ACL name '" << t << "' too long, max " << ACL_NAME_SZ - 1 << " characters supported");
        parser.destruct();
        return;
    }
    ...
    if ((theType = ConfigParser::NextToken()) == nullptr) {
        debugs(28, DBG_CRITICAL, "FATAL: missing ACL type.");
        parser.destruct();
        return;
    }
    ...
    if ((A = FindByName(aclname)) == nullptr) {
        debugs(28, 3, "creating ACL Node '" << aclname << "'");
        A = Make(theType);
        A->context(aclname, config_input_line);
        new_acl = 1;
    } else {
        if (strcmp (A->typeString(),theType) ) {
            debugs(28, DBG_CRITICAL, "FATAL: ACL '" << A->name << "' already exists with different type.");
            parser.destruct();
            return;
        }

        debugs(28, 3, "appending to '" << aclname << "'");
        new_acl = 0;
    }
...
Acl::Node::parseFlags()
{
    // XXX: Performance regression. data copy from const to non-const vector
    ...
    ParseFlags(allOptions);
}

SBufList
Acl::Node::dumpOptions()
{
    SBufList result;
...
Acl::Node::matchForCache(ACLChecklist *)
{
    ...
    fatal("Acl::Node::matchForCache: unknown or unexpected ACL type");
    return 0;       /* NOTREACHED */
}
...
aclCacheMatchFlush(dlink_list * cache)
{
    ...
    debugs(28, 8, "called for cache " << cache);
...
Acl::Node::~Node()
{
    debugs(28, 3, "destructed, this=" << (void*)this << ", name=" << name);
    safe_free(cfgline);
...
Acl::Node::Initialize()
{
    Node *a = Config.aclList;
    debugs(53, 3, "Node::Initialize");
