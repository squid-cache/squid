...
Http::One::ResponseParser::firstLineSize() const
{
    ...
    // protocol name
    // XXX: performance regression. make ProtocolType_str SBuf to avoid strlen()
    result += strlen(AnyP::ProtocolType_str[msgProtocol_.protocol]);
    // version string
    if (msgProtocol_.protocol != AnyP::PROTO_ICY)
        result += 4; // "/N.N" , none on ICY
